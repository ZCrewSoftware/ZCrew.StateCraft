...
/// <summary>
///     A strongly-typed wrapper around an <see cref="IAsyncFunc{TIn, TTuple}"/> producing a three-element value
///     tuple that represents an asynchronous mapping function used by the state machine during a parameter-mapped
///     transition that emits three output parameters.
/// </summary>
/// <remarks>
...
internal readonly record struct AsyncMapValueTuple3<TIn, TOut1, TOut2, TOut3>(
    IAsyncFunc<TIn, (TOut1, TOut2, TOut3)> Map,
    string? Descriptor
)
{
    /// <summary>
    ...
    public Task<(TOut1, TOut2, TOut3)> Invoke(TIn parameter, CancellationToken token)
    {
    ...
    {
        return new MappingFunctionInfo(Descriptor, [typeof(TIn)], [typeof(TOut1), typeof(TOut2), typeof(TOut3)]);
    }
}

/// <summary>
///     A strongly-typed wrapper around an <see cref="IAsyncFunc{TIn1, TIn2, TTuple}"/> producing a three-element
///     value tuple that represents an asynchronous mapping function accepting two input parameters used by the
///     state machine during a parameter-mapped transition that emits three output parameters.
/// </summary>
/// <remarks>
///     This wrapper exists to give mapping functions a distinct nominal type rather than relying on bare delegate
///     types, which allows additional metadata to be associated with mappings in the future without changing call
...
/// <param name="Map">The underlying asynchronous function that is invoked when the mapping runs.</param>
/// <param name="Descriptor">An optional descriptor identifying the mapping, captured from the caller's expression.</param>
internal readonly record struct AsyncMapValueTuple3<TIn1, TIn2, TOut1, TOut2, TOut3>(
    IAsyncFunc<TIn1, TIn2, (TOut1, TOut2, TOut3)> Map,
    string? Descriptor
)
{
    /// <summary>
    ...
    /// <param name="token">The token to monitor for cancellation requests.</param>
    /// <returns>A task that completes with the mapped output value tuple.</returns>
    public Task<(TOut1, TOut2, TOut3)> Invoke(TIn1 parameter1, TIn2 parameter2, CancellationToken token)
    {
    ...
    public IMappingFunctionInfo GetInfo()
    {
        return new MappingFunctionInfo(
            ...
            [typeof(TOut1), typeof(TOut2), typeof(TOut3)]
        );
    }
}
...
///     three-element value tuple that represents an asynchronous mapping function accepting three input
///     parameters used by the state machine during a parameter-mapped transition that emits three output
///     parameters.
/// </summary>
/// <remarks>
///     This wrapper exists to give mapping functions a distinct nominal type rather than relying on bare delegate
...
/// <param name="Map">The underlying asynchronous function that is invoked when the mapping runs.</param>
/// <param name="Descriptor">An optional descriptor identifying the mapping, captured from the caller's expression.</param>
internal readonly record struct AsyncMapValueTuple3<TIn1, TIn2, TIn3, TOut1, TOut2, TOut3>(
    IAsyncFunc<TIn1, TIn2, TIn3, (TOut1, TOut2, TOut3)> Map,
    string? Descriptor
)
{
    /// <summary>
    ...
    /// <param name="token">The token to monitor for cancellation requests.</param>
    /// <returns>A task that completes with the mapped output value tuple.</returns>
    public Task<(TOut1, TOut2, TOut3)> Invoke(
        TIn1 parameter1,
        TIn2 parameter2,
        TIn3 parameter3,
...
///     three-element value tuple that represents an asynchronous mapping function accepting four input parameters
///     used by the state machine during a parameter-mapped transition that emits three output parameters.
/// </summary>
/// <remarks>
///     This wrapper exists to give mapping functions a distinct nominal type rather than relying on bare delegate
///     types, which allows additional metadata to be associated with mappings in the future without changing call
...
/// <param name="Map">The underlying asynchronous function that is invoked when the mapping runs.</param>
/// <param name="Descriptor">An optional descriptor identifying the mapping, captured from the caller's expression.</param>
internal readonly record struct AsyncMapValueTuple3<TIn1, TIn2, TIn3, TIn4, TOut1, TOut2, TOut3>(
    IAsyncFunc<TIn1, TIn2, TIn3, TIn4, (TOut1, TOut2, TOut3)> Map,
    string? Descriptor
)
{
    /// <summary>

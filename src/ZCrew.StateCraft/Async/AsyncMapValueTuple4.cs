...
/// <summary>
///     A strongly-typed wrapper around an <see cref="IAsyncFunc{TIn, TTuple}"/> producing a four-element value
///     tuple that represents an asynchronous mapping function used by the state machine during a parameter-mapped
///     transition that emits four output parameters.
/// </summary>
/// <remarks>
...
internal readonly record struct AsyncMapValueTuple4<TIn, TOut1, TOut2, TOut3, TOut4>(
    IAsyncFunc<TIn, (TOut1, TOut2, TOut3, TOut4)> Map,
    string? Descriptor
)
{
    /// <summary>
...
/// <summary>
///     A strongly-typed wrapper around an <see cref="IAsyncFunc{TIn1, TIn2, TTuple}"/> producing a four-element
///     value tuple that represents an asynchronous mapping function accepting two input parameters used by the
///     state machine during a parameter-mapped transition that emits four output parameters.
/// </summary>
/// <remarks>
///     This wrapper exists to give mapping functions a distinct nominal type rather than relying on bare delegate
///     types, which allows additional metadata to be associated with mappings in the future without changing call
...
/// <param name="Map">The underlying asynchronous function that is invoked when the mapping runs.</param>
/// <param name="Descriptor">An optional descriptor identifying the mapping, captured from the caller's expression.</param>
internal readonly record struct AsyncMapValueTuple4<TIn1, TIn2, TOut1, TOut2, TOut3, TOut4>(
    IAsyncFunc<TIn1, TIn2, (TOut1, TOut2, TOut3, TOut4)> Map,
    string? Descriptor
)
{
    /// <summary>
...
///     four-element value tuple that represents an asynchronous mapping function accepting three input parameters
///     used by the state machine during a parameter-mapped transition that emits four output parameters.
/// </summary>
/// <remarks>
///     This wrapper exists to give mapping functions a distinct nominal type rather than relying on bare delegate
///     types, which allows additional metadata to be associated with mappings in the future without changing call
...
/// <param name="Map">The underlying asynchronous function that is invoked when the mapping runs.</param>
/// <param name="Descriptor">An optional descriptor identifying the mapping, captured from the caller's expression.</param>
internal readonly record struct AsyncMapValueTuple4<TIn1, TIn2, TIn3, TOut1, TOut2, TOut3, TOut4>(
    IAsyncFunc<TIn1, TIn2, TIn3, (TOut1, TOut2, TOut3, TOut4)> Map,
    string? Descriptor
)
{
    /// <summary>
    ...
    /// <param name="token">The token to monitor for cancellation requests.</param>
    /// <returns>A task that completes with the mapped output value tuple.</returns>
    public Task<(TOut1, TOut2, TOut3, TOut4)> Invoke(
        TIn1 parameter1,
        TIn2 parameter2,
        TIn3 parameter3,
...
///     four-element value tuple that represents an asynchronous mapping function accepting four input parameters
///     used by the state machine during a parameter-mapped transition that emits four output parameters.
/// </summary>
/// <remarks>
///     This wrapper exists to give mapping functions a distinct nominal type rather than relying on bare delegate
...
/// <param name="Map">The underlying asynchronous function that is invoked when the mapping runs.</param>
/// <param name="Descriptor">An optional descriptor identifying the mapping, captured from the caller's expression.</param>
internal readonly record struct AsyncMapValueTuple4<TIn1, TIn2, TIn3, TIn4, TOut1, TOut2, TOut3, TOut4>(
    IAsyncFunc<TIn1, TIn2, TIn3, TIn4, (TOut1, TOut2, TOut3, TOut4)> Map,
    string? Descriptor
)
{
    /// <summary>

...
/// <summary>
///     Standard implementation of a state with three parameters.
/// </summary>
[DebuggerDisplay("{DisplayString}")]
internal class State<TState, TTransition, T1, T2, T3> : IState<TState, TTransition>
    where TState : notnull
    where TTransition : notnull
{
    private string DisplayString =>
        $"{StateValue}<{typeof(T1).FriendlyName}, {typeof(T2).FriendlyName}, {typeof(T3).FriendlyName}>";
    ...
    /// <summary>
    ///     Initializes a new instance of the <see cref="State{TState, TTransition, T1, T2, T3}"/> class.
    /// </summary>
    public State(
        TState state,
    ...
    public async Task Activate(IStateMachineParameters parameters, CancellationToken token)
    {
        ...
        foreach (var handler in this.onActivateHandlers)
        {
            await StateMachine.RunWithExceptionHandling(
                () => handler.InvokeAsync(StateValue, p1, p2, p3, token),
                token
            );
        }
    }
    ...
    public async Task Deactivate(IStateMachineParameters parameters, CancellationToken token)
    {
        ...
        StateMachine.Tracker?.Deactivated(this, (p1, p2, p3));
    ...
    )
    {
        ...
        {
            await StateMachine.RunWithExceptionHandling(
                () => handler.InvokeAsync(previousState, transition, StateValue, p1, p2, p3, token),
                token
            );
        }
    }
    ...
    public async Task Enter(IStateMachineParameters parameters, CancellationToken token)
    {
        ...
        {
            await StateMachine.RunWithExceptionHandling(() => handler.InvokeAsync(p1, p2, p3, token), token);
        }
    }
    ...
    {
        var (p1, p2, p3) = parameters.GetCurrentParameters<T1, T2, T3>();
        foreach (var action in this.actions)
        {
            await StateMachine.RunWithExceptionHandling(
                () => action.Invoke(p1, p2, p3, token),

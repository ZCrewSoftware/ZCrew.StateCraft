...
/// <summary>
///     Configures which previous states participate in an inverted transition whose destination state has four
...
///     <see cref="IInitialTransitionConfiguration{TState, TTransition, T1, T2, T3, T4}.From"/> when configuring a
///     transition on a state with four parameters. The state being configured becomes the destination, and
///     <see cref="AllStates"/> or <see cref="AllOtherStates"/> determines which states can transition to it.
/// </remarks>
public interface IFromTransitionConfiguration<TState, TTransition, TNext1, TNext2, TNext3, TNext4>
    where TState : notnull
    where TTransition : notnull
{
    /// <inheritdoc cref="IFromTransitionConfiguration{TState, TTransition}.AllStates"/>
    IFromAllStatesTransitionConfiguration<TState, TTransition, TNext1, TNext2, TNext3, TNext4> AllStates();

    /// <inheritdoc cref="IFromTransitionConfiguration{TState, TTransition}.AllOtherStates"/>
    IFromAllStatesTransitionConfiguration<TState, TTransition, TNext1, TNext2, TNext3, TNext4> AllOtherStates();
}

...
/// <summary>
///     Configures a parameterless transition from a parameterized state. Conditions added here are evaluated after
///     parameter configuration is complete. The next state must be specified using <see cref="To"/>.
/// </summary>
/// <typeparam name="TState">
///     The state type. This should be an <see langword="enum"/> type or it should be an equatable type so the state
...
///     </para>
/// </remarks>
public interface IParameterlessTransitionConfiguration<TState, TTransition, TPrevious>
    where TState : notnull
    where TTransition : notnull
{
    /// <summary>
    ...
    /// <param name="condition">The delegate to check when resolving the transition.</param>
    /// <returns>A reference to the configuration after the configuration was updated.</returns>
    IParameterlessTransitionConfiguration<TState, TTransition, TPrevious> If(Func<bool> condition);
    ...
    /// <param name="condition">The delegate to check when resolving the transition.</param>
    /// <returns>A reference to the configuration after the configuration was updated.</returns>
    IParameterlessTransitionConfiguration<TState, TTransition, TPrevious> If(

...
/// <summary>
///     Configures a parameterized transition from a parameterless state. Conditions added here receive the transition
///     parameter value (which will be passed to the next state). The next state must be specified using
///     <see cref="To"/>.
/// </summary>
...
/// <summary>
///     Configures a parameterized transition from a parameterized state. Conditions added here receive the transition
///     parameter value (which will be passed to the next state). The next state must be specified using
///     <see cref="To"/>.
/// </summary>
/// <typeparam name="TState">
...
///     <para>
///     Conditions added at this stage are evaluated after any conditions added via
///     <see cref="IInitialTransitionConfiguration{TState, TTransition, TPrevious}"/>.
///     </para>
/// </remarks>
    ...
    where TTransition : notnull
{
    ...
    /// <param name="condition">The delegate to check when resolving the transition.</param>
    /// <returns>A reference to the configuration after the configuration was updated.</returns>
    IParameterizedTransitionConfiguration<TState, TTransition, TPrevious, TNext> If(Func<TNext, bool> condition);
    ...
    /// <param name="condition">The delegate to check when resolving the transition.</param>
    /// <returns>A reference to the configuration after the configuration was updated.</returns>
    IParameterizedTransitionConfiguration<TState, TTransition, TPrevious, TNext> If(
        Func<TNext, CancellationToken, Task<bool>> condition
    );

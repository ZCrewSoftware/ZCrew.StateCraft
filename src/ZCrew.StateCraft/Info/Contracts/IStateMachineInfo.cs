...
///     <see cref="IStateInfo{TState, TTransition}"/>.
///     </para>
/// </summary>
/// <remarks>
///     Consumers wanting "the transitions whose source is state X" filter <see cref="Transitions"/> and handle
///     each variant's notion of previous state (see <see cref="ITransitionInfo{TState, TTransition}"/> subtypes).
///     Inverted transitions appear once per declaration, not once per matching source state — see
...
{
    /// <summary>
    ///     The initial state configured on the machine. See <see cref="IStaticInitialStateInfo{TState, TTransition}"/>
    ///     and <see cref="IDynamicInitialStateInfo{TState, TTransition}"/> for the variants.
    /// </summary>
    /// <remarks>
    ...
    /// </summary>
    IReadOnlyList<IStateInfo<TState, TTransition>> States { get; }
    ...
    /// </summary>
    IReadOnlyList<ITransitionInfo<TState, TTransition>> Transitions { get; }
}

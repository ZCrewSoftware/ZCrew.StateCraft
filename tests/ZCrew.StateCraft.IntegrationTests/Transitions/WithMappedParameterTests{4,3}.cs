...
public class WithMappedParameterTests_4_3
{
    [Fact]
            ...
            .WithState(
                ...
                state =>
                    state
                        ...
                        .WithTransition(
                            "To B",
                            t => t.WithMappedParameters<int, string, bool>((a, b, c, _) => (a, b, c)).To("B")
                        )
            )
    ...
    {
        // Arrange
        var mappingFunction = Substitute.For<Func<int, string, bool, double, (int, string, bool)>>();
        mappingFunction
            .Invoke(Arg.Any<int>(), Arg.Any<string>(), Arg.Any<bool>(), Arg.Any<double>())
            .Returns(x => (x.ArgAt<int>(0), x.ArgAt<string>(1), x.ArgAt<bool>(2)));
            ...
            .WithState(
                ...
                state =>
                    state
                        ...
                        .WithTransition(
                            "To B",
                            t => t.WithMappedParameters<int, string, bool>((a, b, c, _) => (a * 2, b + "!", !c)).To("B")
                        )
            )
            .WithState("B", state => state.WithParameters<int, string, bool>().OnEntry(onEntry))
            .Build();
                ...
                state =>
                    state
                        ...
                        .WithTransition(
                            "To B",
                            t =>
                                t.WithMappedParameters<int, string, bool>((a, b, c, _) => (a, b, c))
                                    .If((_, _, _) => true)
                                    .To("B")
                        )
            )
                ...
                state =>
                    state
                        ...
                        .WithTransition(
                            ...
                            t =>
                                t.If((_, _, _, _) => true)
                                    .WithMappedParameters<int, string, bool>((a, b, c, _) => (a, b, c))
                                    .To("B")
                        )
            )
            .WithState("B", state => state.WithParameters<int, string, bool>())
    ...
    public async Task Transition_WithMappedParameters_WithTaskMapping_ShouldTransformParameter()
    {
        // Arrange
        var onEntry = Substitute.For<Action<int, string, bool>>();
        Func<int, string, bool, double, CancellationToken, Task<(int, string, bool)>> mapping = async (a, b, c, _, _) =>
    ...
    public async Task Transition_WithMappedParameters_WithValueTaskMapping_ShouldTransformParameter()
    {
        // Arrange
        var onEntry = Substitute.For<Action<int, string, bool>>();
        Func<int, string, bool, double, CancellationToken, ValueTask<(int, string, bool)>> mapping = (a, b, c, _, _) =>
            ...
            .WithState(
                ...
                state =>
                    state
                        ...
                        .WithTransition(
                            "Loop",
                            t => t.WithMappedParameters<int, string, bool>((a, b, c, _) => (a, b, c)).ToSameState()
                        )
            )

...
public class WithMappedParameterTests_4_4
{
    [Fact]
    ...
    {
        // Arrange
        var mappingFunction = Substitute.For<Func<int, string, bool, double, (int, string, bool, double)>>();
        mappingFunction
            .Invoke(Arg.Any<int>(), Arg.Any<string>(), Arg.Any<bool>(), Arg.Any<double>())
            .Returns(x => (x.ArgAt<int>(0) * 2, x.ArgAt<string>(1) + "!", !x.ArgAt<bool>(2), x.ArgAt<double>(3) * 2));
            ...
            .WithState(
                ...
                state =>
                    state
                        ...
                        .WithTransition(
                            "To B",
                            t => t.WithMappedParameters<int, string, bool, double>((a, b, c, d) => (a, b, c, d)).To("B")
                        )
            )
    ...
    public async Task Transition_WithMappedParameter_ShouldTransformParameter()
    {
        ...
        // Assert
        onEntry.Received(1).Invoke(84, "hello!", false, 6.28);
    }
    ...
    public async Task Transition_WithMappedParameter_ShouldCallOnStateChangeForNextStateWithMappedParameter()
    {
        ...
        // Assert
        onStateChange.Received(1).Invoke("A", "To B", "B", 84, "hello!", false, 6.28);
    }
    ...
    public async Task Transition_WithMappedParameter_ShouldCallActionWithMappedParameter()
    {
        ...
        // Assert
        invoke.Received(1).Invoke(84, "hello!", false, 6.28);
    }
    ...
    public async Task Transition_WithMappedParameter_ShouldCallMethodsInCorrectOrder()
    {
        ...
        Received.InOrder(() =>
        {
            ...
            onStateChangeState.Received(1).Invoke("A", "To B", "B", 84, "hello!", false, 6.28);
            onEntry.Received(1).Invoke(84, "hello!", false, 6.28);
            invoke.Received(1).Invoke(84, "hello!", false, 6.28);
        });
    }
                ...
                state =>
                    state
                        ...
                        .WithTransition(
                            ...
                            t =>
                                t.If((_, _, _, _) => true)
                                    .WithMappedParameters<int, string, bool, double>(
                                        (a, b, c, d) => (a * 2, b + "!", !c, d * 2)
                                    )
                                    .To("B")
                        )
    ...
    public async Task Transition_WithMappedParameters_WithTaskMapping_ShouldTransformParameter()
    {
        // Arrange
        var onEntry = Substitute.For<Action<int, string, bool, double>>();
        Func<int, string, bool, double, CancellationToken, Task<(int, string, bool, double)>> mapping = async (
            a,
            b,
            c,
            d,
            _
        ) => await Task.FromResult((a * 2, b + "!", !c, d * 2));
        var stateMachine = StateMachine
            .Configure<string, string>()
            .WithInitialState("A", 42, "hello", true, 3.14)
            .WithState(
    ...
    public async Task Transition_WithMappedParameters_WithValueTaskMapping_ShouldTransformParameter()
    {
        // Arrange
        var onEntry = Substitute.For<Action<int, string, bool, double>>();
        Func<int, string, bool, double, CancellationToken, ValueTask<(int, string, bool, double)>> mapping = (
            a,
            b,
            c,
            d,
            _
        ) => new ValueTask<(int, string, bool, double)>((a * 2, b + "!", !c, d * 2));
        var stateMachine = StateMachine
            .Configure<string, string>()
            .WithInitialState("A", 42, "hello", true, 3.14)
            .WithState(

...
public class TryTransitionExceptionTests_T1_T2_T3
{
    ...
    [Fact]
    public async Task TryTransition_T1_T2_T3_WhenOnEntryThrowsException_ShouldThrowAndCommit()
    {
        // Arrange
        var exception = new InvalidOperationException("Test exception");
        var stateMachine = StateMachine
    ...
    public async Task TryTransition_T1_T2_T3_WhenOnEntryThrowsExceptionOnce_ShouldRetrySuccessfully()
    {
        ...
        var stateMachine = StateMachine
            ...
            .WithState(
                "B",
                state => state.WithParameters<int, string, bool>().OnEntry(onEntry).WithTransition("To C", "C")
            )
